                ...
                if row[5]=='Counts':
                    ...
                    if int(row[5])>1:
                        if (row[2]) not in dic.keys():      
                            dic[row[2]]=np.ones(15)  
                            dic[row[2]][i-1]=int(row[5])
                        else:
                            dic[row[2]][i-1]+=int(row[5]) 
    return(dic)
...
def g_p(dic):   
    a=['MD**PYLPRPRPPRRIYNR','MD*P*YLPRPRPPRRIYNR','MDKP**LPRPRPPRRIYNR','MDKPP*L*RPRPPRRIYNR','MDK**YLPRPRPPRRIYNR','MDK*P*LPRPRPPRRIYNR','MDKPP**PRPRPPRRIYNR','MDKP*Y*PRPRPPRRIYNR','MDKPPY**RPRPPRRIYNR','MDKPPY*P*PRPPRRIYNR','MDKPPYL**PRPPRRIYNR']
    gp0=12.5
    gp_j={}
    for key in a:
        gp_j[key]=np.zeros(15)
        for i in range(15):
            gp_j[key][i]=gp0-math.log(dic[key][i],2)  #normalized dic already has normalized frequency relative to 0mM population, so no need to re-divide by zero.     
    return gp_j
...
g_p_lst = gp_lst() #bulk number of doubling times for each condition. Same list used in G1_DNA_slope_from_read_counts.py
...
def growth(): 
    ...
    for key in normalized_dic.keys():
        ...
        for i in range(15):
            #if normalized_dic[key][i]==0:    #set floor to one count so no need for checking if normalized is equal to zero
             #   new_dic[key][i]=
            #else:
...
def significance_test(av_dic,st_dic):
    ...
    for key in average_dic.keys():
        if F_test(st_dic[key][0],st_dic['MDKPPYLPRPRPPRRIYNR'][0]):
            #t-test with unequal variance
            t=(av_dic[key][0]-av_dic['MDKPPYLPRPRPPRRIYNR'][0])/(st_dic[key][0]**2/3+st_dic['MDKPPYLPRPRPPRRIYNR'][0]**2/3)**0.5
            dof=(st_dic[key][0]**2/3+st_dic['MDKPPYLPRPRPPRRIYNR'][0]**2/3)**2/(((st_dic[key][0]**2/3)**2)/2+((st_dic['MDKPPYLPRPRPPRRIYNR'][0]**2/3)**2)/2)
            ...
            s_pooled=((2*st_dic[key][0]**2+2*st_dic['MDKPPYLPRPRPPRRIYNR'][0]**2)/4)**0.5
            t=(av_dic[key][0]-av_dic['MDKPPYLPRPRPPRRIYNR'][0])/s_pooled*(9/6)**0.5  
            dof=6-2
            p_value=tstat.cdf(t,dof)
...
def sort(values):
    dtype=[('name', 'U30'), ('pvalue', 'U30'),('average_slope', 'U30'),('average_yint', 'U30'),('std_dev_slope', 'U30'),('std_dev_yint', 'U30')]   #originally said ('std_dev_yint', float)
    a = np.array(values, dtype=dtype)
    return(np.sort(a, order='pvalue'))

sorted_values = sort(p_unsorted) #sort mutants by 1-tailed p-value

nameHandle = open('Gen_1_results_for_AA.csv', 'w') #writing .csv file
